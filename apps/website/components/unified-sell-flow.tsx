...
export function UnifiedSellFlow({ vehicleType, brands, vehicleTypeName, icon }: UnifiedSellFlowProps) {
  ...
  const truckBrands = [
    ...
    { name: "Ashok Leyland", logo: "/placeholder.svg?height=50&width=100" },
  ...
  const renderStepIndicator = () => {
    ...
    const getCurrentStepIndex = () => {
      if (["registration", "brand", "model", "year", "variant", "fuel", "kilometers"].includes(step)) {
        return 0
          ...
          <div key={s.id} className="flex flex-col items-center">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center mb-2 ${
                index <= currentIndex ? "bg-[#4ADE80] text-white" : "bg-gray-300 text-gray-600"
              }`}
            >
              {index + 1}
            </div>
            <span className={index <= currentIndex ? "text-black" : "text-gray-400"}>{s.label}</span>
          </div>
        ))}
      </div>
    )
  ...
  const PriceEstimateSection = () => {
    ...
    return (
      <div className="bg-black text-white p-6 rounded-xl shadow-md">
        ...
        <p className="mb-6">Get an instant estimate based on your vehicle condition</p>
          ...
          <div className="relative h-2 bg-gradient-to-r from-red-500 via-yellow-500 to-green-500 rounded-full mb-8">
            ...
            {conditionOptions.map((option, index) => (
              <div
                ...
                style={{
                  left: index === 0 ? "0%" : index === 1 ? "33.3%" : index === 2 ? "66.6%" : "100%",
        ...
        >
          <h3 className="text-3xl font-bold mb-3 flex items-center">
            ...
            <span className="ml-2 text-green-400 text-lg font-normal">Best Value</span>
          </h3>
    ...
    <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="space-y-8">
          <div className="text-center space-y-4">
            <h1 className="text-3xl font-bold text-black">
              Sell your {vehicleTypeName} <span className="text-green-600">at the best price</span>
            </h1>
            ...
            <div className="text-6xl flex justify-center">
              {typeof icon === "string" ? (
                ...
                <img src="/placeholder.svg?height=80&width=80" alt="Vehicle icon" className="h-20 w-auto" />
              )}
            </div>
          </div>
          ...
          {vehicleType === "4-wheeler" && (
            <div className="max-w-2xl mx-auto space-y-4">
              <h2 className="text-xl font-medium text-center">Select your 4 Wheeler type</h2>
          ...
          <div className="max-w-2xl mx-auto space-y-8">
            ...
            <div className="text-center">
              ...
              <h2 className="text-xl mb-6 text-black">Select your {vehicleTypeName} brand to get started</h2>
      ...
      {step === "model" && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-center">Select {vehicleTypeName} Model</h2>
          <ModelSelector brand={formData.brand} onSelect={handleModelSelect} vehicleType={vehicleType} />
          <Button onClick={() => setStep("registration")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "year" && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-center">Select Manufacturing Year</h2>
          <YearSelector onSelect={handleYearSelect} />
          <Button onClick={() => setStep("model")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "variant" && (
        <div className="space-y-6">
          ...
          <VariantSelector model={formData.model} onSelect={handleVariantSelect} />
          <Button onClick={() => setStep("year")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "fuel" && (
        <div className="space-y-6">
          ...
          <Button onClick={() => setStep("variant")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "kilometers" && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-center">Enter Kilometers Driven</h2>
          <KilometersSelector onSelect={handleKilometersSelect} />
          <Button onClick={() => setStep("fuel")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "city" && (
        <div className="space-y-6">
          ...
          <Button
            onClick={() => setStep(formData.registrationNumber ? "registration" : "kilometers")}
            variant="outline"
            className="mt-4"
          >
            Back
      ...
      {step === "timeline" && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-center">When are you planning to sell?</h2>
          <SellingTimelineSelector onSelect={handleTimelineSelect} />
          <Button onClick={() => setStep("city")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "contact" && (
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-center">Your Contact Information</h2>
          ...
          <Button onClick={() => setStep("timeline")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "otp" && (
        <div className="space-y-6">
          ...
          <OtpVerificationFlow phone={formData.phone} onVerified={handleOtpVerified} />
          <Button onClick={() => setStep("contact")} variant="outline" className="mt-4">
            Back
          </Button>
        </div>
      )}
      ...
      {step === "summary" && (
        <div className="space-y-6">
              ...
              {vehicleType === "4-wheeler" && vehicleSubType && (
                <div>
                  ...
                  <p className="font-semibold">
                    {vehicleSubType === "car" ? "Car" : vehicleSubType === "commercial" ? "Commercial Car" : "Truck"}
                  </p>
                </div>
              )}
              {formData.brand && (
          ...
          <div className="flex justify-between">
            ...
            <Button onClick={handleSubmitSummary} className="bg-[#4ADE80] hover:bg-[#4ADE80]/90">
              Proceed to Checkout
            </Button>
          </div>
        </div>
