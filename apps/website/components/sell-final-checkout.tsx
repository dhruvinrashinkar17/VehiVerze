...
import { CheckCircle, ArrowRight, CreditCard, Wallet, Ban as Bank } from "lucide-react"
...
export function SellFinalCheckout({ vehicleType }: SellFinalCheckoutProps) {
  ...
  // Vehicle details based on the previous steps (would normally come from context or state management)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Store inspection details in localStorage or context for the confirmation page
    ...
    <div className="max-w-5xl mx-auto py-12 px-4">
      ...
      >
        <div className="bg-gradient-to-r from-blue-600 to-green-500 p-6 text-white">
          <h1 className="text-2xl font-bold">Complete Your {vehicleType.replace("-", " ")} Sale</h1>
          <p className="text-blue-100">Please provide your payment details to finalize the sale</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            <div className="md:col-span-3">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Schedule Inspection</h3>
                  <div className="bg-blue-50 p-4 rounded-lg mb-6">
                    <p className="text-sm text-blue-800">
                      <span className="font-medium">Note:</span> Our expert will visit your location to inspect the
                      vehicle on your selected date and time.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      <label htmlFor="inspectionTime" className="block text-sm font-medium text-gray-700 mb-1">
                        Preferred Time
                      </label>
                      <Select
                        value={formData.inspectionTime || ""}
                        onValueChange={(value) => handleSelectChange("inspectionTime", value)}
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="20:00-22:00">08:00 PM - 10:00 PM</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {vehicleType.replace("-", " ")} ({vehicleDetails.year} Model)
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Agreed Price:</span>
                    <span className="font-medium">{formatCurrency(vehicleDetails.agreedPrice)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Service Fee:</span>
                    <span className="font-medium">{formatCurrency(vehicleDetails.serviceFee)}</span>
                  </div>
                  <div className="h-px bg-gray-200 my-2"></div>
                  <div className="flex justify-between text-lg font-bold">
                    <span>Total Payout:</span>
                    <span>{formatCurrency(vehicleDetails.totalPayout)}</span>
                  </div>
                </div>
                  ...
                  <div className="flex">
                    ...
                    <div className="text-sm text-blue-800">
                      ...
                      {formData.inspectionDate && formData.inspectionTime ? (
                        ...
                        <p>Please select your preferred inspection date and time.</p>
                      )}
                    </div>
                  </div>
                </div>
