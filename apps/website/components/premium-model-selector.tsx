...
export function PremiumModelSelector({
  ...
  const popularModels = {
    "4-wheeler": [
      { name: "Baleno", image: "/placeholder.svg?height=100&width=200", popular: true },
      { name: "Swift", image: "/placeholder.svg?height=100&width=200", popular: true },
      { name: "Wagon R 1.0", image: "/placeholder.svg?height=100&width=200", popular: true },
      { name: "Alto 800", image: "/placeholder.svg?height=100&width=200", popular: true },
    ],
    "2-wheeler": [
      { name: "Splendor", image: "/placeholder.svg?height=100&width=200", popular: true },
      { name: "Activa", image: "/placeholder.svg?height=100&width=200", popular: true },
      { name: "Pulsar", image: "/placeholder.svg?height=100&width=200", popular: true },
      { name: "Classic 350", image: "/placeholder.svg?height=100&width=200", popular: true },
    ],
  }
  ...
  useEffect(() => {
    const updatedFilteredModels = searchTerm
      ? allModels.filter((model) => model.name.toLowerCase().includes(searchTerm.toLowerCase()))
      : allModels
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Popular models */}
      {popularModels[vehicleType as keyof typeof popularModels]?.length > 0 && !searchTerm && (
        <div>
          <h3 className="text-xl font-semibold mb-6">POPULAR {brand.toUpperCase()} MODELS</h3>
          <motion.div
          ...
          >
            {popularModels[vehicleType as keyof typeof popularModels].map((model) => (
              <motion.div
                key={model.name}
                variants={itemVariants}
                whileHover={{ y: -5 }}
                animate={selectedModel === model.name ? "selected" : "initial"}
              ...
              >
                ...
                <div className="p-4">
                  <h4 className="text-lg font-medium text-gray-800">{model.name}</h4>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      {/* All/filtered models */}
      <div>
        <h3 className="text-xl font-semibold mb-6">{searchTerm ? "SEARCH RESULTS" : "ALL MODELS"}</h3>
