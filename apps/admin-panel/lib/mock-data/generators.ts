...
import type { VehicleType, LeadStatus, VendorStatus, PaymentStatus, NotificationType, CreditType } from "./types"
...
const vehicleModels = {
  ...
  "3 Wheeler": ["Bajaj RE", "Piaggio Ape", "Mahindra Alfa", "TVS King", "Atul Gem", "Force Minidor", "Bajaj Maxima"],
  "4 Wheeler - Cars": [
    "Maruti Swift",
    "Hyundai i20",
    "Tata Nexon",
  ...
  "4 Wheeler - Trucks": ["Tata 407", "Mahindra Jayo", "Eicher Pro 1049", "BharatBenz 914R", "Ashok Leyland Partner"],
  "6 Wheeler": [
    "Tata 1109",
...
export function generateLead() {
  const type = faker.helpers.arrayElement(Object.keys(vehicleModels) as VehicleType[])
  const status: LeadStatus = faker.helpers.arrayElement(["New", "Contacted", "Qualified", "Converted", "Failed"])
  const serviceType = faker.helpers.arrayElement(["Buy", "Sell", "Garage Service", "New Vehicle"])
...
export function generateVendor() {
  return {
    ...
    status: faker.helpers.arrayElement(["Active", "Inactive", "Pending Verification", "Suspended"] as VendorStatus[]),
    rating: faker.number.float({ min: 3, max: 5, precision: 0.1 }),
    totalOrders: faker.number.int({ min: 10, max: 1000 }),
    completionRate: faker.number.float({ min: 75, max: 99, precision: 0.1 }),
...
export function generatePayment() {
  return {
    ...
    status: faker.helpers.arrayElement(["Completed", "Pending", "Failed", "Refunded"] as PaymentStatus[]),
    orderId: faker.string.alphanumeric(8).toUpperCase(),
    customerName: faker.person.fullName(),
    paymentMethod: faker.helpers.arrayElement(["UPI", "Net Banking", "Credit Card", "Debit Card", "Cash"]),
...
export function generateNotification() {
  ...
  return {
    ...
    targetUsers: faker.helpers.arrayElements(["All", "Vendors", "Customers", "Admins"], { min: 1, max: 2 }),
  }
}
