...
export function generateLiveBiddingSession(): LiveBiddingSession {
  ...
  const highestBid = bids.length > 0 ? Math.max(...bids.map((b) => b.amount)) : baseBid
...
export function generateLiveBiddingSessions(count: number): LiveBiddingSession[] {
...
export const biddingDb = {
    ...
    biddingSessions = biddingSessions.map((session) =>
      session.id === sessionId
        ? {
            ...
            winningVendorId: bid.amount > session.highestBid ? bid.vendorId : session.winningVendorId,
            winningVendorName: bid.amount > session.highestBid ? bid.vendorName : session.winningVendorName,
          }
        : session,
    )
  ...
  extendBidding: (sessionId: string, newEndTime: string) => {
    biddingSessions = biddingSessions.map((session) =>
      session.id === sessionId ? { ...session, biddingEndTime: newEndTime } : session,
    )
    return biddingSessions.find((session) => session.id === sessionId)
  },
}
