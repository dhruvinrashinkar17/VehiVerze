...
import { getInspectionQuestions, conditionMultipliers } from "@/lib/mock-data/inspection-questions"
...
export default function FailedOrderDetailPage() {
  ...
  inspectionQuestions.forEach((question) => {
    const conditions: VehicleCondition[] = ["Excellent", "Very Good", "Good", "Average", "Poor"]
    mockInspectionResults[question.id] = conditions[Math.floor(Math.random() * conditions.length)]
  })
  ...
  const calculateCategoryTotals = (categoryQuestions: typeof inspectionQuestions) => {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <div className="text-sm text-gray-400">Physical Condition</div>
            ...
            <div className="flex gap-2">
              <Button variant="destructive" className="bg-red-600 hover:bg-red-700" onClick={handleAcceptFail}>
                Accept Fail
              </Button>
              <Button variant="default" className="bg-green-600 hover:bg-green-700" onClick={handleReverseLead}>
                Reverse Lead
              </Button>
          ...
          <Card className="bg-[#1A1A1A] border-[#2A2A2A]">
            ...
            <CardContent>
              <div className="p-4 bg-red-500/10 rounded-lg border border-red-500/20">
                <h4 className="font-semibold text-red-500 mb-2">Sold Device Elsewhere</h4>
                <p className="text-sm text-gray-400">
                  Customer has sold the Vehicle through another platform or dealer.
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Detailed pricing breakdown based on vehicle condition assessment
                </p>
              </div>
              <div className="text-right">
                <div className="text-sm text-gray-400">Total Deduction</div>
                <div className="text-2xl font-bold text-red-500">-₹{totalDeduction.toLocaleString()}</div>
                <div className="text-xs text-gray-500 mt-1">Adjusted Price: ₹{adjustedPrice.toLocaleString()}</div>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid grid-cols-4 gap-4 p-4 bg-gray-800/50 rounded-lg">
              <div className="text-center">
                ...
                <div className="text-lg font-bold">₹{basePrice.toLocaleString()}</div>
              </div>
              <div className="text-center">
                <div className="text-sm text-gray-400">Price Per Point</div>
                <div className="text-lg font-bold">₹{pricePerPoint.toFixed(2)}</div>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <div className="text-lg font-bold text-green-500">₹{adjustedPrice.toLocaleString()}</div>
              </div>
            </div>

            <div className="space-y-3">
              {Object.entries(questionsByCategory).map(([category, categoryQuestions]) => {
                const { totalDeduction: categoryDeduction, totalPoints: categoryPoints } =
                  ...
                  >
                    <CollapsibleTrigger className="flex items-center justify-between w-full p-4 bg-gray-800/50 rounded-lg hover:bg-gray-800/70 transition-colors">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <div className="text-xs text-gray-400">Category Deduction</div>
                          <div className="text-lg font-bold text-red-400">-₹{categoryDeduction.toLocaleString()}</div>
                        </div>
                      </div>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="mt-2">
                      <div className="space-y-2 p-4 bg-gray-900/30 rounded-lg">
                        <div className="grid grid-cols-12 gap-2 pb-2 border-b border-gray-700 text-xs font-semibold text-gray-400">
                          ...
                          <div className="col-span-2 text-right">Base Value</div>
                          <div className="col-span-2 text-right">Actual Value</div>
                        ...
                        {categoryQuestions.map((question, index) => {
                          ...
                          const conditionColors: Record<VehicleCondition, string> = {
                            ...
                            Average: "bg-orange-500/20 text-orange-400 border-orange-500/30",
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="col-span-2 flex items-center justify-center">
                                <Badge className={conditionColors[condition]} variant="outline">
                                  {condition}
                                </Badge>
                              </div>
                              ...
                              <div className="col-span-1 flex items-center justify-end font-mono">
                                {deduction > 0 ? (
                                  <span className="text-red-400">-₹{deduction.toFixed(0)}</span>
                                ) : (
                        ...
                        <div className="grid grid-cols-12 gap-2 pt-2 mt-2 border-t border-gray-700 font-semibold">
                          <div className="col-span-7 flex items-center justify-end text-gray-300">Category Total:</div>
            ...
            <div className="p-6 bg-gradient-to-r from-gray-800/50 to-gray-900/50 rounded-lg border border-gray-700">
              <div className="grid grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="text-sm text-gray-400 mb-1">Original Quote</div>
                  <div className="text-2xl font-bold text-gray-300">₹{basePrice.toLocaleString()}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-gray-400 mb-1">Total Deductions</div>
                  <div className="text-2xl font-bold text-red-400">-₹{totalDeduction.toLocaleString()}</div>
                  <div className="text-xs text-gray-500 mt-1">
                    ({((totalDeduction / basePrice) * 100).toFixed(1)}% reduction)
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-gray-400 mb-1">Final Adjusted Price</div>
                  <div className="text-2xl font-bold text-green-400">₹{adjustedPrice.toLocaleString()}</div>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="bg-yellow-900/20 border-yellow-600/30">
          <CardContent className="pt-6">
            <div className="text-center text-yellow-400">
              <p>300-Point Inspection Pricing Report is only available for "Sell" service type orders.</p>
