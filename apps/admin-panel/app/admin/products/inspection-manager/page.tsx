...
export default function InspectionManagerPage() {
  const [vehicles, setVehicles] = useState(() =>
    vehiclesDb.getAll().filter((v) => v.serviceType === "Sell")
  );
  const [selectedVehicle, setSelectedVehicle] = useState<VehicleProduct | null>(
    null
  );
  const [viewInspection, setViewInspection] = useState<VehicleProduct | null>(
    null
  );
  const [editingVehicle, setEditingVehicle] = useState<VehicleProduct | null>(
    null
  );
  ...
  const calculateCurrentResults = () => {
    ...
    const adjustedPrice = calculatePriceAdjustment(
      customBasePrice,
      overallCondition
    );
  ...
  const handleSaveInspection = () => {
    ...
    const { totalScore, maxScore, condition, adjustedPrice, results } =
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">
          Vehicle Inspection & Price Manager
        </h1>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {vehicles.filter((v) => v.inspectionDate).length}
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(() => {
                const inspected = vehicles.filter(
                  (v) => v.inspectionDate && v.currentPrice !== v.basePrice
                );
                if (inspected.length === 0) return "0%";
                const avgAdjustment =
                  inspected.reduce((sum, v) => {
                    return (
                      sum + ((v.currentPrice - v.basePrice) / v.basePrice) * 100
                    );
                  }, 0) / inspected.length;
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="4 Wheeler - Cars">
                  4 Wheeler - Cars
                </SelectItem>
                <SelectItem value="4 Wheeler - Commercial Cars">
                  4 Wheeler - Commercial Cars
                </SelectItem>
                <SelectItem value="4 Wheeler - Trucks">
                  4 Wheeler - Trucks
                </SelectItem>
                <SelectItem value="6 Wheeler">6 Wheeler</SelectItem>
                <SelectItem value="More Than 8 Wheelers">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setFilters((prev) => ({ ...prev, inspectionStatus: value }))
              }
            >
              <SelectTrigger className="w-[180px]">
                        ...
                        <div className="space-y-1">
                          ...
                          {vehicle.inspectionScore && (
                            <div className="text-xs text-muted-foreground">
                              {vehicle.inspectionScore}/
                              {vehicle.maxInspectionScore || 300}
                            </div>
                          )}
                        </div>
                      ) : (
      ...
      {editingVehicle && (
        <Card className="border-2 border-primary">
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>
                300-Point Inspection: {editingVehicle.brand}{" "}
                      ...
                      ([category, categoryQuestions]) => {
                        ...
                        const categoryScore = categoryQuestions.reduce(
                          (sum, q) => {
                            ...
                            return (
                              sum +
                              Math.round(q.points * conditionPoints[condition])
                            );
                          },
                          0
                        );
      ...
      >
        <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedVehicle?.brand} {selectedVehicle?.model} - Complete
              inspection to determine accurate pricing
            </DialogDescription>
          </DialogHeader>
          {selectedVehicle && (
      ...
      <Dialog
        open={!!viewInspection}
        onOpenChange={() => setViewInspection(null)}
      >
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-4 bg-muted rounded-lg">
                  ...
                  <div className="text-2xl font-bold">
                    {viewInspection.inspectionScore || 0}/
                    {viewInspection.maxInspectionScore || 300}
                  </div>
                </div>
                <div className="text-center p-4 bg-muted rounded-lg">
                  <div className="text-sm text-muted-foreground">Condition</div>
                  <Badge
                    className={
                      conditionColors[viewInspection.condition || "Good"]
                    }
                  >
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="font-medium">
                      ₹{viewInspection.basePrice.toLocaleString()}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-muted-foreground">
                    ...
                    <div className="font-medium text-primary">
                      ₹
                      {(
                        viewInspection.currentPrice || viewInspection.basePrice
                      ).toLocaleString()}
                    </div>
                  </div>
                </div>
