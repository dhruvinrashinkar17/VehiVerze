    ...
    <div className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10 flex-shrink-0 shadow-sm">
      <div className="w-full bg-card">
        <div className="flex items-center justify-between gap-2 sm:gap-4 px-4 sm:px-6 py-3 lg:py-4">
          {deviceType !== "desktop" && (
            <Button variant="ghost" size="icon" onClick={onMenuClick} className="h-9 w-9 flex-shrink-0 hover:bg-accent">
          ...
          <div className="relative flex-1 min-w-0 max-w-full sm:max-w-md">
            ...
            <Input
              placeholder={deviceType === "mobile" ? "Search..." : "Search orders, leads, vendors..."}
          ...
          {/* Desktop Filters */}
          {user?.role === "admin" && deviceType !== "mobile" && (
            <div className="hidden lg:flex items-center gap-2">
              ...
              <DatePickerWithRange
                ...
                onApply={(date) => setFilters((prev) => ({ ...prev, dateRange: date }))}
              />
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="h-9 w-9 flex-shrink-0 hover:bg-accent relative">
              <Bell className="h-5 w-5" />
              <span className="absolute top-1 right-1 h-2 w-2 bg-destructive rounded-full"></span>
            </Button>

            <Button variant="ghost" size="icon" className="h-9 w-9 flex-shrink-0 hover:bg-accent">
        ...
        {/* Mobile Filters */}
        {user?.role === "admin" && deviceType === "mobile" && filtersOpen && (
