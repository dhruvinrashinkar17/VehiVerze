...
export function DetailedInspectionForm({
  vehicleType,
  basePrice,
  onInspectionComplete,
}: DetailedInspectionFormProps) {
  ...
  if (!activeTab && categories.length > 0 && categories[0]) {
  ...
  const { results, totalScore, maxScore, overallCondition, adjustedPrice } =
    calculateCurrentResults();
  const completionPercentage =
    (Object.keys(inspectionResults).length / questions.length) * 100;

  const getCategoryProgress = (categoryQuestions: typeof questions) => {
    const answered = categoryQuestions.filter(
      (q) => inspectionResults[q.id]
    ).length;
      ...
      <Card>
        <CardContent className="text-center py-8">
          <div className="text-muted-foreground">
            No inspection questions available for {vehicleType}.
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline">
              {Object.keys(inspectionResults).length} / {questions.length}{" "}
              Completed
            </Badge>
          </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-4 bg-muted rounded-lg">
              <Badge
                className={conditionColors[overallCondition]}
                variant="secondary"
              >
                {overallCondition}
              </Badge>
              ...
              <TabsContent key={category} value={category} className="p-6">
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">
                      {questionsByCategory[category]?.length ?? 0} Questions
                    </Badge>
                  </div>

                  <div className="space-y-4">
                    {(questionsByCategory[category] ?? []).map(
                      (question, index) => (
                              ...
                              <div className="grid grid-cols-5 gap-2">
                                {(
                                  ...
                                  >
                                    <RadioGroupItem
                                      value={condition}
                                      id={`${question.id}-${condition}`}
                                    />
                                    <Label
                                      htmlFor={`${question.id}-${condition}`}
      ...
      <div className="flex justify-end gap-4">
        ...
        <Button
          ...
          disabled={Object.keys(inspectionResults).length === 0}
        >
